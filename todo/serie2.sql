...
-- 3
SELECT e.last_name FROM employee e JOIN department d ON e.department_id = d.id where LOWER(d."name") = 'distribution';
...
-- 5
SELECT * FROM employee WHERE department_id IN (SELECT id FROM department WHERE LOWER(name) LIKE 'finance');
...
-- 11
SELECT last_name, title, salary FROM employee where title in (select title from employee where last_name = 'fairant') and salary in (select salary from employee where last_name = 'fairant');

-- 12
SELECT e.department_id, d.name, e.last_name FROM employee e right join department d on e.department_id = d.id order by d.id;

-- 13
SELECT AVG(salary) FROM employee WHERE title LIKE 'secrétaire';

-- 14
select COUNT(title) FROM employee group by title;
